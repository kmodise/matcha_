  ...
  while (i <= 500) {
      ...
      sql = 'INSERT INTO `users` (`username`, `firstname` , `name`, `password`, `email`, `confirmkey`, `confirm`, `gender`, `orientation`, `bio`, `age`, `score`, `profileImg`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      conn.query(sql, [username, firstname, name, password, email, key, confirm, gender, orientation, bio, age, score, profileImg], function (err, result) { if (err) throw err })
